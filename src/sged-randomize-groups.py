...
for grp in range(n_groups):
    size = int(
        df_groups.loc[grp, "Size"]
    )  # TODO: if the size column is not present, we should generate it
    ...
    if len(gp) != size:
        raise IOError(
            "!!! Error in input file, group size does not match number or sites! %i vs.%i"
            % (len(gp), size)
        )
